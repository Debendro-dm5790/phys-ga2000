...
for numPoints in numPointsArray:
    ...
    for t in range(len(tList)):
        ...
        if t == np.int32(len(tList)//9) or t == 2 * np.int32(len(tList)//9) or t == 3 * np.int32(len(tList)//9) or t == 4 * np.int32(len(tList)//9):
            savedTimes.append(currentTime)
            '''
            Save Density Data
            '''
            filename = 'SpaceInvertDensityDataAtTime=' + str(round(currentTime,3)) + 'withSpatialGridSize=' + str(numPoints) + '.csv'
            fields = np.array(['Position (meters)', 'Mass Density (kg per cubic meter)'])
            Save.SaveDataAsCSVFile(xList, u1[1:len(u1)-1], filename, fields)
            fileList.append(filename)
            print(filename)
            ...
            Save Velocity Data
            '''
            filename = 'SpaceInvertVelocityDataAtTime=' + str(round(currentTime,3)) + 'withSpatialGridSize=' + str(numPoints) + '.csv'
            fields = np.array(['Position (meters)', 'Velocity (meters per second)'])
            Save.SaveDataAsCSVFile(xList, u2[1:len(u1)-1]/u1[1:len(u1)-1], filename, fields)
            fileList.append(filename)
            print(filename)
            ...
            Save Pressure Data
            '''
            filename = 'SpaceInvertPressureDataAtTime=' + str(round(currentTime,3)) + 'withSpatialGridSize=' + str(numPoints) + '.csv'
            fields = np.array(['Position (meters)', 'Pressure (Pascals)'])
            Save.SaveDataAsCSVFile(xList, (gamma - 1)*(u3[1:len(u1)-1] - 0.5*(u2[1:len(u1)-1]**2/u1[1:len(u1)-1])), filename, fields)
            fileList.append(filename)
            print(filename)
...
for i in range(len(savedTimes)):
    savedTimes[i] = round(savedTimes[i],3)
